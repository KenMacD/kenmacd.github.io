...
function Bulletin()
{
   ...
   bgv_op = 100;
   bgv_timeout = 0;
            setButtonClass("scalableButton");
            if (bgv_timeout > 0) setTimeout(closeBulletin, bgv_timeout*1000);

 
   bdivTop = (getWinHeight() - 200) / 2;


bdivRight = getWinWidth() * (bgv_right / 100);
...
if (isMobile())
   ...
   bulletinWrite('</style>');
   bulletinWrite('<style>');
   bulletinWrite('   img.scalableButton {');

if (isMobile())
         
   bulletinWrite('        max-width : 60%;');
   ...
   bulletinWrite('    }');
   bulletinWrite('</style>');
   bulletinWrite('         <DIV width="30%" class="scalable btn" style="border:6px solid #332a86; border-radius:20px; background:' + (bgv_bg) + '; opacity:' + (bgv_op/100.0) + '; filter:Alpha(opacity=' + (bgv_op) + ');">');
   bulletinWrite('            <TABLE align="center" width="410px" bgcolor="white" cellpadding="4" style="border-radius:20px">');
   bulletinWrite('               <TD align="center">             ');
   bulletinWrite('                  <font face="Arial" size="5" color="332a86"><b>Copyright Notice:</b><br></font>');
   bulletinWrite('                  </TD>');
   bulletinWrite('               </tr>');
   bulletinWrite('               <TR>');
   bulletinWrite('                  <TD align="center">                     ');
   bulletinWrite('                        <font face="Arial" size="4" color="332a86">Eastlink has received notice that your Internet service may have been used to illegally download or share copyrighted material.  This message is being forwarded to you in accordance with Canada\'s copyright legislation. <b>');
makeButton("SeeNotice", "Nav", "~https://notice.eastlink.ca/notice/", "!See Notice Here");
   bulletinWrite('</b></font>');
   bulletinWrite('                  </TD>');
   bulletinWrite('               </TR>');
   bulletinWrite('               <TR>');
   bulletinWrite('                  <TD align="center">');
   bulletinWrite('                     <TABLE align="center">');
   bulletinWrite('                        <TR>');
   bulletinWrite('                        </TR>');
   bulletinWrite('                     </TABLE>');
   bulletinWrite('                  </TD>');
